...
(defn attendees-input [value]
  [:input {:type "number"
           :class "w-16 border-2 border-indigo-500 p-2 mt-2 md:mt-0 rounded"
...
(defn duration-input [value]
  [:input {:type "number"
           :class "w-24 border-2 border-indigo-500 p-2 rounded"
...
(defn home-page []
  (let [{:keys [attendees duration context meeting business note]} @meeting-data]
    [:div {:class "mx-4 sm:mx-auto max-w-xl font-mono my-2"}
